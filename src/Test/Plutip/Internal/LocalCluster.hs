...
import Servant.Client (BaseUrl (BaseUrl), Scheme (Http), runClientM, mkClientEnv)
...
import Cardano.Wallet.Primitive.Types (NetworkParameters(NetworkParameters), SlottingParameters (SlottingParameters), SlotLength (SlotLength))
import qualified Cardano.BM.Configuration as CM
import qualified Plutus.ChainIndex.Config as CIC
import Ledger.TimeSlot (scSlotLength)
import Plutus.ChainIndex (Tip(Tip))
import Ledger (Slot(Slot))
import qualified Plutus.ChainIndex.Client as ChainIndexClient
...
waitForRelayNode trCluster rn = do
  ...
  where
    ...
    wait _ = do
      traceWith trCluster WaitingRelayNode
      tip <- Tools.queryTip rn
      case tip of
        (CAPI.ChainTip (CAPI.SlotNo _) _ _) -> pure ()
