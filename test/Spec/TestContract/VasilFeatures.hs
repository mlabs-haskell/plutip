...
testSendInlineDatum = do
  ...
  let correctDatumCheck :: ChainIndexTxOut -> Bool
      correctDatumCheck txOut =
        Just expectedDatum == do
          DatumInline d <- snd <$> txOut ^? ciTxOutScriptDatum
          pure d
...
hasRefScriptCheck :: Validator -> ChainIndexTxOut -> Bool
hasRefScriptCheck validator txOut =
  Just validator == do
    Validator . Scripts.unversioned <$> txOut ^. ciTxOutReferenceScript
